...
from .config import CACHE_DIR, CONF_DIR, MODULE_DIR
...
def list_out():
    ...
    try:
        last_used_theme = utils.read_file(os.path.join(CACHE_DIR, "last_used_theme"))[
            0
        ].replace(".json", "")
...
def get_random_theme(dark: bool = True):
...
def file(input_file, light: bool = False):
    ...
    bright = "light" if light else "dark"

    user_theme_file = os.path.join(CONF_DIR, "colorschemes", bright, theme_name)
    theme_file = os.path.join(MODULE_DIR, "colorschemes", bright, theme_name)
    ...
    logging.error(f"No {bright} colorscheme file found.")
