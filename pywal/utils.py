...
def brighten_color(value: str, amount: int | float, brighten: bool) -> str:
    amount = int(amount * 100) if type(amount) is float else amount
    color = tuple(
        min(255, col + amount) if brighten else max(1, col - amount)
        for col in hex_to_rgb(value)
    )
...
def get_pid(name: str) -> bool:
    ...
    try:
        subprocess.check_output(
            ["pidof", "-s", name]
        ) if system() != "Darwin" else subprocess.check_output(["pidof", name])
